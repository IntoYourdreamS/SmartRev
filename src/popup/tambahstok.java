...
public class tambahstok extends javax.swing.JFrame {
   ...
   String idBarang = id_barang.getText().trim();
    ...
    Date tanggalExp = tgl_exp.getDate();

    // Validasi input
    if (idBarang.isEmpty() || namaBarang.isEmpty() || kodeBarcode.isEmpty() || 
        tanggalExp == null || hargaBeliStr.isEmpty()) {
        JOptionPane.showMessageDialog(this, "Semua field harus diisi termasuk tanggal expired dan harga beli!", 
    ...
    try {
        ...
        String idPembelian = generateCode1();
        
        // Get current date
        java.sql.Date tanggalPembelian = new java.sql.Date(System.currentTimeMillis());
        
        // Get default supplier (you may want to change this)
        String idSupplier = "SP003"; 
        // Get current employee ID (you may want to get this from session)
        String idKaryawan = "KR001"; 
        ...
        if (updated > 0) {
            // 5. Tambah data ke tabel pembelian
            PreparedStatement pstPembelian = conn.prepareStatement(
                "INSERT INTO pembelian (id_pembelian, id_karyawan, id_supplier, tanggal) VALUES (?, ?, ?, ?)");
            pstPembelian.setString(1, idPembelian);
            pstPembelian.setString(2, idKaryawan);
            pstPembelian.setString(3, idSupplier);
            pstPembelian.setDate(4, tanggalPembelian);
            pstPembelian.executeUpdate();
            
            // 6. Tambah data ke tabel detail_pembelian
            PreparedStatement pstDetailPembelian = conn.prepareStatement(
                "INSERT INTO detail_pembelian (id_pembelian, id_produk, jumlah, sub_total, harga_beli, kategori) " +
                "VALUES (?, ?, 1, ?, ?, (SELECT kategori FROM produk WHERE id_produk = ?))");
            pstDetailPembelian.setString(1, idPembelian);
            pstDetailPembelian.setString(2, idBarang);
            pstDetailPembelian.setInt(3, hargaBeli); // sub_total = harga_beli * jumlah (1)
            pstDetailPembelian.setInt(4, hargaBeli);
            pstDetailPembelian.setString(5, idBarang);
            pstDetailPembelian.executeUpdate();
            
            conn.commit(); // Commit transaksi
            JOptionPane.showMessageDialog(this, 
                "Barcode berhasil ditambahkan, stok bertambah 1, dan data pembelian tercatat!", 
        ...
        JOptionPane.showMessageDialog(this, "Harga beli harus berupa angka!", "Kesalahan", JOptionPane.ERROR_MESSAGE);
    }
